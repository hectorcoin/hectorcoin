...
#if defined(NDEBUG)
# error "Hectorcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4974a71270374066549764d3ca89d02e8ee87bb9fca2814e7e61871cd25dfc4a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hectorcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hectorcoin Signed Message:\n";
...
{
    // Hectorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 50000); // Hectorcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6 * 60; // Hectorcoin: 3.5 days
static const int64 nTargetSpacing = 6 * 60; // Hectorcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hectorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hectorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x98bca704128944d613be3d895db2f4086966a8fb2502813d69ed3bb4b4c4dee0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Andrea Leadsom quits over Brexit leaving deluded Theresa May clinging on to power as Cabinet deserts her";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 484375  * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0405b75d20cc801d88811207727636c406ba12d38a0ff654c1027fe28edcc176d9ea7d0c08b55b517fd8d37c8d7378f5e34c99c068231728c84b7a46a855c3cc33") << OP_CHECKSIG;                                                          
        ...
        block.nVersion = 1;
        block.nTime    = 1558590552;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3240251;

        if (fTestNet)
        {
            block.nTime    = 1558590537;
            block.nNonce   = 2338353;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1e4ba9bbae4b23738009ee92c3709d42d5a573aa3268f5f43bd6dba41b87e358"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc2, 0xba, 0xdc }; // Hectorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HectorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HectorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HectorcoinMiner(CWallet *pwallet)
{
    printf("HectorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hectorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HectorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HectorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HectorcoinMiner, pwallet));
}
